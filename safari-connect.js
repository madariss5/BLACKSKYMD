/**
 * Safari Connection Method for WhatsApp Bot
 * This script uses Safari browser fingerprinting for better connection success rate in cloud environments
 */
...
}, pino.destination(path.join(LOGS_FOLDER, 'safari-connection.log')));

// Copy existing auth files if available
function copyExistingAuth() {
    const sources = ['./auth_info_baileys', './auth_info_terminal', './auth_info_qr'];
...
async function connectToWhatsApp() {
    ...
    // Connection configuration with Safari fingerprint
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr) {
            ...
            if (qrAttempts >= MAX_QR_ATTEMPTS) {
                ...
                logger.info('Consider using another connection method or check your internet connection.');
...
╭───────────────────────────────────────────────╮
│                                               │
│    Safari Connection Method for WhatsApp      │
│    Optimized for cloud environments (Replit)  │
│                                               │
│    • Using Safari browser fingerprint         │
